	...
	private void updateListView() {
		if (mActivity != null && mListView != null) {
			...
			for (ZoneInfo zoneInfo : zones) {
				mHeaderItems.add(new HeaderItem(zoneInfo.getZoneName()));
				ArrayList<ListItem> mZoneItems = new ArrayList<ListItem>();
				for (Fence fence : zoneInfo.getFences()) {
					boolean isCheckedIn = ApplicationSharedPreferencesHelper
							.getInstance(mActivity).isCheckedInFence(fence);
